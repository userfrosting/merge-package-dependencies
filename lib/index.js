 ...
 * 
 * @param {object} package Package object to validate
 * @param {string} packageSpec Used to determine what dependency keys should be merged.
 ...
 * @private
 */
function npmValidate(package, packageSpec, log) {
    let results = {
        valid: true
    };
    ...
    if (package.private == true) {
        ...
        for (let depType of depTypes) {
            if (!_.isUndefined(package[depType])) {
                if (!_.isObject(package[depType])) {
    ...
    else results = packageJsonValidator(JSON.stringify(package));
