...
let yarnDependencyTypes = [
    "dependencies",
    "devDependencies",
    "resolutions"
];
let bowerDependencyTypes = [
    "dependencies",
    "devDependencies",
    "resolutions"
];
...
function packageJsonMerge(template, paths, saveTo, log, packageSpec) {
    ...
    if (packageSpec == "npm") {
        ...
        dependencyTypes = yarnDependencyTypes;
    }
    ...
    // Perform merge.
    template = mergePackageDependencies(template, packages, dependencyTypes, log);
...
function bowerMerge(template, paths, saveTo = null, log = false) {
    ...
    // Perform merge.
    template = mergePackageDependencies(template, packages, bowerDependencyTypes, log);
...
function mergePackageDependencies(tml, pkgs, depTypes, log) {
    ...
    for (let pkg of pkgs) {
        if (log) {
            log(`Starting merge of package '${pkg.name}'`);
        }
        ...
        if (log) {
            log(`Finished merge of package '${pkg.name}'`);
        }
    }
